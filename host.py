...
def main():
    ...
    srv.bind((HOST, PORT))
    srv.listen(8)
    print(f"[HOST] Listening on {HOST}:{PORT} ... waiting for Arranger and Chooser clients")
    ...
    def handle_handshake(sock: socket.socket, addr):
        ...
        if role not in ("Arranger", "Chooser"):
            try:
                send_msg(sock, {"type": "ERROR", "reason": "Role must be Arranger or Chooser"})
            except Exception:
                pass
            sock.close()
            return
        with lock:
            if role in roles:
                try:
                    send_msg(sock, {"type": "ERROR", "reason": f"Role {role} already taken"})
                except Exception:
                    pass
                sock.close()
                return
    ...
    def accept_loop():
        while True:
            try:
                ...
                break
            threading.Thread(target=handle_handshake, args=(sock, addr), daemon=True).start()
    ...
    print(f"[HOST] Sent S_pool to Chooser ({chooser_name}). Waiting up to 120s for S'...")
    t0 = time.time()
    try:
        msg = recv_msg(chooser_sock, timeout=120.0)
    except socket.timeout:
        print("[HOST] Chooser did not respond in time. Arranger wins by timeout.")
        try: send_msg(chooser_sock, {"type": "RESULT", "winner": "Arranger", "reason": "Chooser timeout on ordering S'"})
        except Exception: pass
        try: send_msg(arranger_sock, {"type": "RESULT", "winner": "Arranger", "reason": "Chooser timeout on ordering S'"})
        except Exception: pass
        return
    except Exception as e:
        print(f"[HOST] Chooser connection error: {e}")
        try: send_msg(arranger_sock, {"type": "RESULT", "winner": "Arranger", "reason": "Chooser connection error"})
        except Exception: pass
        return
    elapsed = time.time() - t0
    if msg.get("type") != "ORDER_RESPONSE":
        print("[HOST] Invalid response from Chooser.")
        try: send_msg(chooser_sock, {"type": "ERROR", "reason": "Expected ORDER_RESPONSE"})
        except Exception: pass
        return
    S_prime = msg.get("S_prime", [])
    if not isinstance(S_prime, list) or sorted(S_prime) != sorted(S_pool):
        try: send_msg(chooser_sock, {"type": "ERROR", "reason": "S_prime must be a reordering of S_pool"})
        except Exception: pass
        return
    print(f"[HOST] Received S' in {elapsed:.2f}s from Chooser. Forwarding to Arranger...")
    ...
    try:
        ...
        try: send_msg(chooser_sock, {"type": "RESULT", "winner": "Chooser", "reason": "Arranger connection error"})
        except Exception: pass
        return
    if msg2.get("type") != "ARRANGE_RESPONSE":
        try: send_msg(arranger_sock, {"type": "ERROR", "reason": "Expected ARRANGE_RESPONSE"})
        except Exception: pass
        return
    final_seq = msg2.get("sequence", [])
    if not isinstance(final_seq, list) or len(final_seq) != 16:
        try: send_msg(arranger_sock, {"type": "ERROR", "reason": "Sequence must be list of 16 ints"})
        except Exception: pass
        return
    ...
    try:
        ...
        try: send_msg(arranger_sock, {"type": "RESULT", "winner": "Arranger", "reason": "Chooser disconnected before picking"})
        except Exception: pass
        return
    if pick_msg.get("type") != "PICK_START_RESPONSE":
        try: send_msg(chooser_sock, {"type": "ERROR", "reason": "Expected PICK_START_RESPONSE"})
        except Exception: pass
        return
    start_num = pick_msg.get("start", None)
    if not isinstance(start_num, int) or not (1 <= start_num <= 8):
        try: send_msg(chooser_sock, {"type": "ERROR", "reason": "Start must be integer 1..8"})
        except Exception: pass
        return
    ...
    for role in ("Chooser", "Arranger"):
        try: send_msg(roles[role][0], result_payload)
        except Exception: pass
